...
<div class="row row_container_form">
	<div class="row">
     <h3><?= $this->getDI()->get("translate")->_($title) ?><?= ' - ' ?><?= $menu_name ?></h3>
	</div>
	<hr></hr>
	<div class="row">
	<!-- BEGIN FORM-->
	  ...
	  <?php if ($item['name'] == 'image_path') { ?>
		...
		<label name="lbllogo" id="lblloko" class="control-label col-md-1 formlabel">
			<a href="#ModalEditor" id="logourl"  data-toggle="modal" ><i class="fa fa-file-image-o"></i>
				<?= ' ' ?><?= $this->getDI()->get("translate")->_('Image') ?> </a>
		</label>
		<div class="col-md-2">
		...
		<div id ="logo_image" class="col-md-2">
			...
			<img id="theImg" src="<?= $this->url->get('files/images/' . $image_path) ?>" width="50px" heigh="50px"/>
			<?php } ?>
		</div>
	</div>
		<?php } else { ?>
	...
	<?php } ?>
       <div class="col-md-offset-1 col-md-3" style="padding-left:0;">
		...
		<?= $this->tag->linkTo([$routelist . '/' . $menuid, $this->getDI()->get("translate")->_($cancel_button_name), 'class' => 'btn btn-default']) ?>
       </div>
    </div>   
	<!-- FORM ACTION BUTTONS-->
	</form>
...
categoryid:'<?= $this->getDI()->get("translate")->_('dish.category.required') ?>'
,name:'<?= $this->getDI()->get("translate")->_('dish.name.required') ?>'
,price:'<?= $this->getDI()->get("translate")->_('dish.price.required') ?>'
,price_number:'<?= $this->getDI()->get("translate")->_('dish.price.number') ?>'
};
</script>
