  ...
  <?php } ?>
	...
	<?php if (isset($page->items)) { ?>
		<?php foreach ($page->items as $entity) { ?>
			...
			<?php } ?>
			<td width ="2%">
			<?php if ($special_permission['add_user_role'] == 'Y') { ?>
				<?php echo $this->tag->linkTo(array('userrole/list/' . $entity->id, '<i class="fa fa-shield fa-lg"></i>', 'class' => 'btn btn-icon-only blue')); ?>
      <?php } ?>
			</td>
      <td width ="2%">
			<?php if ($special_permission['change_password'] == 'Y') { ?>
			<?php echo $this->tag->linkTo(array('user/set_password_change/' . $entity->id, '<i class="fa fa-lock fa-lg"></i>', 'class' => 'btn btn-icon-only yellow')); ?>
      <?php } ?>
			</td>
			<td width ="2%">
