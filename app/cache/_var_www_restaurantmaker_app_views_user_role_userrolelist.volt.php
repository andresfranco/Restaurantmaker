...
<div class="col-sm-12 col-md-2 col-xs-12 col-lg-2 accordion_menu">
  ...
  <div class="main_content">
   ...
   <h3 class="page-title" align ="left"><?= $this->getDI()->get("translate")->_($title) ?> (<?= $username ?>)</h3>
   <div align="right"><a href ="<?= $this->url->get('user/list') ?>" class="btn btn blue"><?= $this->getDI()->get("translate")->_('Usuarios') ?> <i class="fa fa-arrow-right "></i> </a></div>
	<hr/>
  <!-- GRID SEARCH -->
	<div align="left" >
  ...
  <!-- END GRID SEARCH-->
	 ...
	 <!-- NEW ITEM ICON-->
	<div align="left"><?= $this->tag->linkTo([$newroute . '/' . $userid, '<i class="fa fa-plus fa-lg"></i>']) ?></div>
	...
	<?php if (isset($page->items)) { ?>
		<?php foreach ($page->items as $entity) { ?>
			...
			<?php } ?>
			<td width ="2%"><?= $this->tag->linkTo([$showroute . $entity->userid . '-' . $entity->roleid, '<i class="fa fa-remove fa-lg"></i>', 'class' => 'btn btn-icon-only red']) ?></td>
			</tr>
		<?php } ?>
		<?php } ?>
		</tbody>
