...
<div class="col-sm-12 col-md-2 col-xs-12 col-lg-2 accordion_menu">
  ...
  <?php } ?>
	...
	<!-- GRID BODY -->
		...
		<tr>
		<?php foreach ($headercolumns as $index => $item) { ?>
		<?php if ($item['column_name'] == 'flag') { ?> 
		<td width ="10%"><img src ="<?php echo $this->url->getStatic('metronic/assets/global/img/flags/' . $entity->readAttribute($item['column_name'])); ?>"></img></td>
		<?php } else { ?>
		...
		<?php echo $this->tag->linkTo(array($editroute . $entity->code, '<i class="fa fa-edit fa-lg"></i>', 'class' => 'btn btn-icon-only green')); ?>
		<?php } ?>
		</td>

		<td width ="2%">
		<?php if ($permissions['delete'] == 'Y') { ?>
		<?php echo $this->tag->linkTo(array($showroute . $entity->code, '<i class="fa fa-remove fa-lg"></i>', 'class' => 'btn btn-icon-only red')); ?>
		<?php } ?>
		</td>
		</tr>
		<?php } ?>
