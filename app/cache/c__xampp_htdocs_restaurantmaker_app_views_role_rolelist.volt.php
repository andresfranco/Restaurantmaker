  ...
  <?php } ?>
	...
	<?php if (isset($page->items)) { ?>
		<?php foreach ($page->items as $entity) { ?>
			...
			<?php } ?>
			<td width ="2%">
				<?php if ($special_permission['add_role_action'] == 'Y') { ?>
				<?php echo $this->tag->linkTo(array('actionrole/list/' . $entity->id, '<i class="fa fa-shield fa-lg"></i>', 'class' => 'btn btn-icon-only blue')); ?>
				<?php } ?>
			</td>
			<td width ="2%">
