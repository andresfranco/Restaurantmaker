  ...
  <?php } ?>
	...
	<?php if (isset($page->items)) { ?>
		<?php foreach ($page->items as $entity) { ?>
			<tr>
			<?php foreach ($headercolumns as $index => $item) { ?>
        <?php if ($item['column_name'] == 'active') { ?>
				   <?php if ($entity->readAttribute($item['column_name']) == 'Y') { ?>
					    <td width ="15%"><?= $this->getDI()->get("translate")->_('Yes') ?></td>
             <?php } elseif ($entity->readAttribute($item['column_name']) == 'N') { ?>
						  <td width ="15%"><?= $this->getDI()->get("translate")->_('No') ?></td>
					 <?php } ?>
			  <?php } else { ?>
        ...
        <?php } ?>
			...
			<td width ="2%">
				...
				<?= $this->tag->linkTo(['article_translation/list/' . $entity->id, '<i class="fa fa-language fa-lg"></i>', 'class' => 'btn btn-icon-only yellow']) ?>
				<?php } ?>
			</td>
			<td width ="2%">
				<?php if ($permissions['edit'] == 'Y') { ?>
