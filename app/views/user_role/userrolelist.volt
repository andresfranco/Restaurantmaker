...
{% block content %}
  ...
  <!-- END GRID SEARCH-->
	...
	<li class="ms-hover">
	<a href="{{ url('userrole/list')~'/'~userid~'?page='~page.current~'&order='~item['column_name']~' asc'}}">
	...
	<li class="ms-hover">
	<a href="{{url('userrole/list')~'/'~userid~'?page='~page.current~'&order='~item['column_name']~' desc'}}">
	...
	{% if page.items is defined %}
		{% for entity in page.items %}
			...
			{% endfor %}
			<td width ="2%">{{link_to(showroute~entity.userid~'-'~entity.roleid,'<i class="fa fa-remove fa-lg"></i>','class':'btn btn-icon-only red')}}</td>
			</tr>
		{% endfor %}
		{% endif %}
		</tbody>
