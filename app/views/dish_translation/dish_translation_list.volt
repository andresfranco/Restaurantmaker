...
{% block content %}
  ...
  <!-- END GRID SEARCH-->
	...
	{% if permissions['create']=='Y' %}
	 <!-- NEW ITEM ICON-->
	<div align="left">{{ link_to(newroute~'/'~dishid,'<i class="fa fa-plus fa-lg"></i>')}}</div>
  {% endif %}
	<br>
	{% if noitems ==""%}
	...
	{% if page.items is defined %}
		{% for entity in page.items %}
			...
			<td width ="2%">
				...
				{{link_to(editroute~entity.id~'/'~dishid,'<i class="fa fa-edit fa-lg"></i>','class':'btn btn-icon-only green')}}
				{% endif %}
			</td>
			<td width ="2%">
	...
	<!--END GRID BODY -->
		...
		<div align ="left">
		<ul class="pagination">
		<li>{{ link_to(listroute~'/'~dishid,'<i class="fa fa-angle-left"></i><i class="fa fa-angle-left"></i>') }}</li>
		<li>{{ link_to(listroute~'/'~dishid~"?page="~page.before, '<i class="fa fa-angle-left"></i>') }}</li>
		...
		{% endfor %}
		<li>{{ link_to(listroute~'/'~dishid~"?page="~page.next, '<i class="fa fa-angle-right"></i>') }}</li>
		<li>{{ link_to(listroute~'/'~dishid~"?page="~page.last, '<i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i>') }}</li>
		</ul>
		</div>
    <!--END GRID PAGINATION -->
	{% else %}
