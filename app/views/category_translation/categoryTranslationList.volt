...
{% block content %}
   <h3 class="page-title" align ="left">{{title|t}} ({{category}})</h3><div align="right"><a href ="{{url('dish_category/list' )}}" class="btn btn blue">{{'Categories'|t}} <i class="fa fa-arrow-right "></i> </a></div>
	<hr/>
  <!-- GRID SEARCH -->
	<div align="left" >
	{{ form(searchroute~'/'~categoryId, "method":"post", "autocomplete" : "off") }}
	<div class="row">
  ...
  <!-- END GRID SEARCH-->
	...
	{% if permissions['create']=='Y' %}
	 <!-- NEW ITEM ICON-->
	<div align="left">{{ link_to(newroute~'/'~categoryId,'<i class="fa fa-plus fa-lg"></i>')}}</div>
  {% endif %}
	<br>
	{% if noitems ==""%}
	...
	{% if page.items is defined %}
		{% for entity in page.items %}
			...
			<td width ="2%">
				...
				{{link_to(editroute~entity.id~'/'~categoryId,'<i class="fa fa-edit fa-lg"></i>','class':'btn btn-icon-only green')}}
				{% endif %}
			</td>
			<td width ="2%">
	...
	<!--END GRID BODY -->
		...
		<div align ="left">
		<ul class="pagination">
		<li>{{ link_to(listroute~'/'~categoryId,'<i class="fa fa-angle-left"></i><i class="fa fa-angle-left"></i>') }}</li>
		<li>{{ link_to(listroute~'/'~categoryId~"?page="~page.before, '<i class="fa fa-angle-left"></i>') }}</li>
		...
		{% set classitem ='' %}
		{% endif %}
		<li class="{{classitem}}">{{ link_to(listroute~'/'~categoryId~"?page="~i, i) }}</li>
		{% endfor %}
		<li>{{ link_to(listroute~'/'~categoryId~"?page="~page.next, '<i class="fa fa-angle-right"></i>') }}</li>
		<li>{{ link_to(listroute~'/'~categoryId~"?page="~page.last, '<i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i>') }}</li>
		</ul>
		</div>
    <!--END GRID PAGINATION -->
	{% else %}
